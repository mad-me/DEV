...
Rectangle {
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            PlatformCard {
                cardData: cardBolt
                platformName: "Bolt"
                isVisible: cardBolt.zeile1 !== "-" && parseFloat(cardBolt.zeile1) > 0
            }
            // 31300 Card
            PlatformCard {
                cardData: card31300
            ...
            ColumnLayout {
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            TextField {
                                ...
                                // Silence-Period: Verhindert Events in den ersten 200ms nach Laden
                                property bool silencePeriodActive: true
                                
                                onTextChanged: {
                                    if (abrechnungsBackend && !silencePeriodActive) {
                                ...
                                // Silence-Period Timer: Deaktiviert Events für 200ms nach Laden
                                Timer {
                                    id: silenceTimer
                                    interval: 200 // 200ms Stille nach Laden
                                    running: true  // Startet automatisch
                                    onTriggered: {
                                        console.log("DEBUG: Silence-Period beendet - Events wieder aktiv");
                                        inputField1.silencePeriodActive = false;
                                    }
                                }
                
                // Timer für Ergebnis-Update Debouncing (verhindert doppelte Berechnungen)
                Timer {
                    id: ergebnisUpdateTimer
                    interval: 200 // 200ms Verzögerung
                    onTriggered: {
                        console.log("DEBUG: Debounced Tank-Update, berechne Ergebnis");
                        ...
                        RowLayout {
                            ...
                            TextField {
                                ...
                                Timer {
                                    ...
                                    interval: 500 // Verlängert auf 500ms für bessere Benutzererfahrung
                                    onTriggered: {
                                        if (abrechnungsBackend) {
                                            var currentText = inputField2.text
                                            abrechnungsBackend.inputEinsteiger = currentText
                                            
                                            // Prüfe ob ein gültiger Wert > 0 eingegeben wurde
                                            var numericValue = parseFloat(currentText.replace(',', '.'))
                                            var shouldCalculate = !isNaN(numericValue) && numericValue > 0
                ...
                Rectangle {
                    ...
                    // MouseArea für Click-Funktionalität
                    MouseArea {
                        anchors.fill: parent
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        ...
                        onClicked: {
                            console.log("Ergebnis geklickt:", abrechnungsBackend.abrechnungsergebnis.toFixed(2), "€")
                            // Hier können Sie die gewünschte Aktion hinzufügen
                            // Beispiel: abrechnungsBackend.onResultClicked()
                        }
    ...
    function calculateOverlayAnteil() {
        ...
        // DEBUG: Anteil-Berechnung (Encoding-Probleme entfernt)
        return anteil;
    }
    ...
    Connections {
        ...
        function onDealChanged() {
            // Ergebnis automatisch neu berechnen wenn sich der Deal-Typ ändert (debounced)
            ergebnisUpdateTimer.restart();
        }
        ...
        function onInputGasChanged() {
            ...
            // Debouncing: Verzögerte Neuberechnung um doppelte Berechnungen zu vermeiden
            ergebnisUpdateTimer.restart();
        }
        ...
        function onHeadcardGarageChanged() {
            ...
            // Ergebnis automatisch neu berechnen wenn sich Garage-Wert ändert (debounced)
            ergebnisUpdateTimer.restart();
        }
        ...
        function onInputEinsteigerChanged() {
            ...
            // Ergebnis automatisch neu berechnen wenn sich Einsteiger-Wert ändert (debounced)
            ergebnisUpdateTimer.restart();
        }
    }

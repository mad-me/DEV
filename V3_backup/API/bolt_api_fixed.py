#!/usr/bin/env python3
"""
Bolt API Fixed Implementation
Verwendet die korrekte Client ID und Authentifizierung
"""
...
class BoltAPIFixed:
    ...
    def load_config(self):
        ...
        if self.config_file.exists():
            ...
            # Standard-Konfiguration basierend auf Session-Daten
            self.config = {
                "client_id": "bolt_fleet_portal",  # Standard Client ID für Fleet Portal
                "client_secret": "",
                "company_id": "30496",  # Aus Session extrahiert
    ...
    def get_access_token(self):
        ...
        try:
            ...
            # Extrahiere Token und Company ID aus Session
            token = None
            company_id = None
                ...
                if origin.get("origin") == "https://fleets.bolt.eu":
                    for item in origin.get("localStorage", []):
                        if item.get("name") == "taxifyFleetOwnerPortal_refresh_token":
                            ...
                            company_id = item.get("value")
            
            if token and company_id:
                # Update config mit Company ID aus Session
                ...
                print(f"✅ Bolt Token aus Session extrahiert (Company ID: {company_id})")
                return token
            
            print("❌ Kein Token oder Company ID in Session gefunden")
            return None
    ...
    def get_reports_data(self, start_date: str = None, end_date: str = None):
        ...
        # Versuche verschiedene Endpunkte mit korrekter Company ID
        endpoints = [
            f"api/companies/{self.config['company_id']}/reports",
            f"api/reports",
            f"api/fleet/reports",
            f"api/companies/{self.config['company_id']}/fleet/reports",
            f"api/v1/companies/{self.config['company_id']}/reports",
            f"api/v1/reports"
        ]
    ...
    def get_dashboard_data(self):
        ...
        endpoints = [
            ...
            "api/dashboard",
            "api/fleet/dashboard",
            f"api/v1/companies/{self.config['company_id']}/dashboard",
            f"api/v1/dashboard"
        ]
    ...
    def get_performance_data(self):
        ...
        endpoints = [
            ...
            f"api/v1/companies/{self.config['company_id']}/performance",
    ...
    def download_csv_report(self, start_date: str, end_date: str, save_path: str = None):
        ...
        endpoints = [
            ...
            f"api/v1/companies/{self.config['company_id']}/reports/csv",
...
def main():
    ...
    print("🔗 Bolt API Fixed Implementation")
    print("=" * 40)
